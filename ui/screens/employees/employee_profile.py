"""
ui/screens/employees/employee_profile.py
=========================================
بطاقة الموظف الكاملة — صورة + بيانات + مستندات + رفع ملفات.
تُفتح عند الضغط على 👁 عرض في قائمة الموظفين.
"""
...
class EmployeeProfileScreen(ctk.CTkFrame):
    ...
    employee_id : int
    on_back     : Callable — العودة لقائمة الموظفين
    on_edit     : Callable[[int], None] — فتح نموذج التعديل
    """
    ...
    def _build(self) -> None:
        ...
        ctk.CTkButton(
            bar, text="✏️  تعديل البيانات",
            font=FONTS["btn_sm"], width=140, height=36,
            corner_radius=RADIUS["md"],
        ...
        photo_frame.grid(row=0, column=0, padx=24, pady=24, sticky="n")

        self._photo_lbl = ctk.CTkLabel(
            photo_frame, text="",
        ...
        ctk.CTkButton(
            photo_frame, text="📷 تغيير الصورة",
            font=FONTS["caption"], width=110, height=28,
            corner_radius=RADIUS["sm"],
            fg_color=COLORS["surface3"], hover_color=COLORS["accent_dim"],
            text_color=COLORS["text_muted"],
        ...
        ctk.CTkLabel(
            info, text=emp.get("position", "—"),
            font=FONTS["subtitle"], text_color=COLORS["text_muted"], anchor="w",
        ...
        _Badge(badges, emp.get("employee_code", ""), COLORS["accent_dim"],    COLORS["accent"]).pack(side="left", padx=(0, 8))
        _Badge(badges, emp.get("department", ""),   COLORS["surface3"],       COLORS["text"]).pack(side="left", padx=(0, 8))
        _Badge(badges, STATUS_LABELS.get(st, st),   COLORS["success_dim"] if st=="active" else COLORS["error_dim"], STATUS_COLORS.get(st, COLORS["text_muted"])).pack(side="left")
        ...
        # Upload new doc
        upload_bar = ctk.CTkFrame(scroll, fg_color="transparent")
        upload_bar.grid(row=2, column=0, sticky="ew", pady=(0, 12))
        upload_bar.grid_columnconfigure(0, weight=1)
        ...
        ctk.CTkOptionMenu(
            ...
            height=38, corner_radius=RADIUS["md"], width=220,
        ).grid(row=0, column=0, sticky="w")

        ctk.CTkButton(
            upload_bar, text="📎  رفع مستند",
    ...
    def _render_photo(self) -> None:
        ...
        try:
            if path and os.path.exists(path):
                ...
                self._photo_lbl._image = ctk_img   # prevent GC
                return
        except Exception:
            pass
        # Fallback avatar
    ...
    def _upload_photo(self) -> None:
        ...
        if not path:
            return
        dest_dir = UPLOAD_DIR / "photos" / "employees"
        dest_dir.mkdir(parents=True, exist_ok=True)
        ext  = pathlib.Path(path).suffix
        name = f"{self._emp_id}_photo{ext}"
        dest = dest_dir / name
    ...
    def _upload_document(self) -> None:
        ...
        if not path:
            return
        dest_dir = UPLOAD_DIR / "documents" / "employees"
        dest_dir.mkdir(parents=True, exist_ok=True)
        fname = pathlib.Path(path).name
        dest  = dest_dir / f"{self._emp_id}_{fname}"
        shutil.copy2(path, dest)
    ...
    def _delete_doc(self, doc: dict) -> None:
        with CRUDService() as svc:
            ...
            svc.delete_document(doc["id"], file_store=store)
            self._docs = svc.list_documents(self._emp_id)
        self._render_docs()
...
class _DocRow(ctk.CTkFrame):
    def __init__(self, parent, doc: dict, on_delete: Callable) -> None:
        super().__init__(
            parent,
        ...
        ctk.CTkLabel(
            self, text=icon,
            font=FONTS["body"], text_color=COLORS["text"], width=160,
        ).grid(row=0, column=0, padx=14, pady=10, sticky="w")

        ctk.CTkLabel(
            self, text=doc.get("file_name", "—"),
            font=FONTS["body"], text_color=COLORS["text"], anchor="w",
        ).grid(row=0, column=1, padx=8, pady=10, sticky="w")

        ctk.CTkLabel(
            self,
            text=doc.get("uploaded_at", "")[:10],
...
class _InfoCell(ctk.CTkFrame):
    def __init__(self, parent, label: str, value: str) -> None:
        super().__init__(
            parent, fg_color=COLORS["surface2"],
            corner_radius=RADIUS["md"],
        )
        ctk.CTkLabel(
            self, text=label,
...
class _Badge(ctk.CTkLabel):
    def __init__(self, parent, text, bg, fg) -> None:
        super().__init__(
            parent, text=f"  {text}  ",
            font=FONTS["body_sm"], text_color=fg,
            fg_color=bg, corner_radius=RADIUS["sm"],
            height=26,
        )
