"""
ui/screens/employees/employee_form.py — V2.1
...
DEPARTMENTS = [
    "الإدارة العليا", "الموارد البشرية", "المالية والمحاسبة",
    "المبيعات", "العمليات", "تقنية المعلومات", "أخرى",
]
...
class EmployeeFormScreen(ctk.CTkFrame):
    ...
    def _build(self) -> None:
        ...
        if self._is_edit and self._existing:
            ctk.CTkLabel(header, text=self._existing.get("employee_code",""),
        ...
        self._f_first = Field(scroll, "الاسم الأول *", self._first_name, placeholder="مثال: أحمد")
        self._f_first.grid(row=1, column=0, sticky="ew", padx=(0,12), pady=(0,12))
        self._f_last = Field(scroll, "الاسم الأخير *", self._last_name, placeholder="مثال: محمد")
        self._f_last.grid(row=1, column=1, sticky="ew", pady=(0,12))
        self._f_nat_id = Field(scroll, "رقم الهوية *", self._nat_id, placeholder="مثال: 1098765432")
        self._f_nat_id.grid(row=2, column=0, sticky="ew", padx=(0,12), pady=(0,12))
        self._f_dob = Field(scroll, "تاريخ الميلاد *", self._dob, placeholder="YYYY-MM-DD")
        self._f_dob.grid(row=2, column=1, sticky="ew", pady=(0,12))

        gf = ctk.CTkFrame(scroll, fg_color="transparent")
        gf.grid(row=3, column=0, sticky="ew", padx=(0,12), pady=(0,16))
        ctk.CTkLabel(gf, text="الجنس", font=FONTS["label"],
                     text_color=COLORS["text_muted"], anchor="w").pack(fill="x", pady=(0,4))
        ctk.CTkOptionMenu(gf, variable=self._gender_var, values=GENDER_OPTIONS,
        ...
        Divider(scroll).grid(row=4, column=0, columnspan=2, sticky="ew", pady=(0,16))
        ...
        self._f_phone = Field(scroll, "الهاتف", self._phone, placeholder="0501234567")
        self._f_phone.grid(row=6, column=0, sticky="ew", padx=(0,12), pady=(0,12))
        Field(scroll, "البريد الإلكتروني", self._email,
              placeholder="ahmed@company.com").grid(row=6, column=1, sticky="ew", pady=(0,12))
        Field(scroll, "العنوان", self._address,
              placeholder="الرياض، حي النزهة").grid(
            row=7, column=0, columnspan=2, sticky="ew", pady=(0,12))

        Divider(scroll).grid(row=8, column=0, columnspan=2, sticky="ew", pady=(0,16))
        ...
        df.grid(row=10, column=0, sticky="ew", padx=(0,12), pady=(0,12))
        ctk.CTkLabel(df, text="القسم *", font=FONTS["label"],
                     text_color=COLORS["text_muted"], anchor="w").pack(fill="x", pady=(0,4))
        ctk.CTkOptionMenu(df, variable=self._dept_var, values=DEPARTMENTS,
                          font=FONTS["body"], fg_color=COLORS["surface2"],
                          button_color=COLORS["surface3"],
                          button_hover_color=COLORS["accent"],
                          text_color=COLORS["text"], height=40,
        ...
        ctk.CTkLabel(sf, text="الحالة الوظيفية", font=FONTS["label"],
                     text_color=COLORS["text_muted"], anchor="w").pack(fill="x", pady=(0,4))
        ctk.CTkOptionMenu(sf, variable=self._status_var, values=STATUS_OPTIONS,
                          font=FONTS["body"], fg_color=COLORS["surface2"],
                          button_color=COLORS["surface3"],
                          button_hover_color=COLORS["accent"],
                          text_color=COLORS["text"], height=40,
        ...
        Divider(scroll).grid(row=12, column=0, columnspan=2, sticky="ew", pady=(0,16))

        _SectionLabel(scroll, "Notes").grid(
            row=13, column=0, columnspan=2, sticky="w", pady=(0,8))
        self._notes_box = ctk.CTkTextbox(
            scroll, height=80, fg_color=COLORS["surface2"],
        ...
        ctk.CTkButton(btn_row, text="Cancel", font=FONTS["btn_sm"],
                      width=100, height=40, corner_radius=RADIUS["md"],
                      fg_color=COLORS["surface3"], hover_color=COLORS["surface2"],
                      text_color=COLORS["text_muted"], command=self._on_cancel,
                      ).pack(side="left", padx=(0,12))
        self._save_btn = ctk.CTkButton(
            btn_row, text="Save Changes" if self._is_edit else "Add Employee",
            font=FONTS["btn_sm"], width=140, height=40, corner_radius=RADIUS["md"],
            fg_color=COLORS["accent"], hover_color=COLORS["accent_hover"],
            text_color=COLORS["text_inverse"], command=self._submit,
        )
        self._save_btn.pack(side="left")

        self._err_banner = ctk.CTkLabel(
            scroll, text="", font=FONTS["body_sm"],
    ...
    def _submit(self) -> None:
        ...
        ok = True
        if not first:    self._f_first.show_error("مطلوب");    ok = False
        if not last:     self._f_last.show_error("مطلوب");     ok = False
        if not nat_id:   self._f_nat_id.show_error("مطلوب");  ok = False
        if not position: self._f_position.show_error("مطلوب"); ok = False

        parsed_dob  = _parse_date(dob)  if dob  else None
        parsed_hire = _parse_date(hire) if hire else None
        if dob  and not parsed_dob:  self._f_dob.show_error("صيغة خاطئة");  ok = False
        if not hire:                 self._f_hire.show_error("مطلوب");       ok = False
        elif not parsed_hire:        self._f_hire.show_error("صيغة خاطئة"); ok = False

        if not ok:
            return

        data = dict(
            first_name=first, last_name=last, national_id=nat_id,
            date_of_birth=parsed_dob, gender=self._gender_var.get(),
            phone=self._phone.get().strip() or None,
            email=self._email.get().strip() or None,
            address=self._address.get().strip() or None,
            department=self._dept_var.get(), position=position,
            hire_date=parsed_hire, employment_status=self._status_var.get(),
            notes=self._notes_box.get("1.0","end").strip() or None,
        )
        try:
            with self._ctx.db() as svc:
                emp = svc.update_employee(self._employee_id, data) \
                    if self._is_edit else svc.create_employee(data)
    ...
    def _show_banner(self, msg: str) -> None:
        ...
        self._err_banner.grid(row=16, column=0, columnspan=2, sticky="ew", pady=(0,8))
