"""
ui/screens/main_window.py — V2.1
يستخدم WorkspaceContext لتمرير db_path لكل الشاشات.
"""
...
from ui.theme import COLORS, FONTS, RADIUS, SIDEBAR_WIDTH, TOPBAR_HEIGHT, WINDOW_MIN_W, WINDOW_MIN_H
...
from ui.screens.storage.backup_manager     import BackupManagerScreen
from ui.screens.storage.data_location_manager import DataLocationManager
...
class MainWindow(ctk.CTk):
    def __init__(self, config: dict, workspace: Workspace,
        ...
        super().__init__()
        # ── Context موحد لكل الشاشات ─────────────────────────
        self._ctx       = WorkspaceContext(workspace)
        self._on_logout = on_logout
        self._frames: dict[str, ctk.CTkFrame] = {}

        ctk.set_appearance_mode("dark")
        self.title(f"MZz System V2.1  —  {config.get('company_name','')}")
    ...
    def _build(self) -> None:
        ...
        self._sidebar = Sidebar(
            ...
            company_name=self._ctx.config.get("company_name", "MZz System"),
        )
    ...
    def _navigate(self, key: str) -> None:
        ...
        titles = {
            ...
            "storage":   "موقع البيانات",
            "settings":  "الإعدادات",
        }
        self._topbar.set_title(titles.get(key, key))

    def _build_screen(self, key: str) -> ctk.CTkFrame:
        ctx = self._ctx   # اختصار

        if key == "dashboard":
            return DashboardScreen(self._content, ctx)
        ...
        if key == "vehicles":
            return VehiclesScreen(self._content, ctx)

        if key == "reports":
            return ReportsScreen(self._content, ctx)

        if key == "backup":
            return BackupManagerScreen(self._content, ctx)

        if key == "storage":
            return DataLocationManager(
                self._content, ctx,
    ...
    def _employees_add(self) -> None:
        self._show_employee_form(None)

    def _employees_edit(self, eid: int) -> None:
        self._show_employee_form(eid)

    def _employees_view(self, eid: int) -> None:
        self._show_employee_profile(eid)
    ...
    def _persons_add(self) -> None:
        self._show_person_form(None)

    def _persons_edit(self, pid: int) -> None:
        self._show_person_form(pid)

    def _persons_view(self, pid: int) -> None:
        self._show_person_form(pid)
    ...
    def _on_config_change(self, new_config: dict) -> None:
        self._ctx.save_config(new_config)
        self.title(f"MZz System V2.1  —  {new_config.get('company_name','')}")
        self._ctx.log("Config updated from settings.")

    # ── Logout ────────────────────────────────────────────────

    def _logout(self) -> None:
        self._ctx.log("User logged out.")
        self.destroy()
        self._on_logout()
...
class _Topbar(ctk.CTkFrame):
    def __init__(self, parent, title: str, username: str) -> None:
        super().__init__(parent, height=TOPBAR_HEIGHT,
        ...
        ctk.CTkLabel(inner, textvariable=self._title_var,
                     font=FONTS["heading"], text_color=COLORS["text"],
                     anchor="w").pack(side="left")

        ctk.CTkLabel(inner, text=f"👤  {username}",
...
class _ComingSoon(ctk.CTkFrame):
    def __init__(self, parent, name: str) -> None:
        ...
        ctk.CTkLabel(self, text=f"🚧  {name}", font=FONTS["title"],
                     text_color=COLORS["text_muted"]).pack(expand=True)
        ctk.CTkLabel(self, text="قريباً في الإصدار القادم.",
                     font=FONTS["body"], text_color=COLORS["text_dim"]).pack()